...
class GUI:
    ...
    def __init__(self, quads: dict):
        """
        quad_list = {
            'quad_1_name':
                {
                    'position':quad_1_position,
                    'orientation':quad_1_orientation,
                    'arm_span':quad_1_arm_span
                },
            ...
        }
        :param quads: Dict of quadrotors
        """
        self.quads = quads
        self.points = dict()
        self.fig = plt.figure()  # type: plt.Figure
    ...
    def add_point(self, name: str):
        self.points[name] = dict()
        self.points[name]['point'] = np.zeros((3, 1))
        self.points[name]['plt'], = self.ax.plot([], [], [], marker='o', color='red', markersize=2,
                                                antialiased=False)
    ...
    def update(self):
        ...
        for key in self.points:
            point = self.points[key]['point'].reshape(3, 1)
            self.points[key]['plt'].set_data(point[0:2, :])
            self.points[key]['plt'].set_3d_properties(point[2, :])
